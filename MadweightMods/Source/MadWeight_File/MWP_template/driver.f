...
C***** Modification made by Andres Osorio: Sept. 2012 

      OPEN(UNIT=25,FILE='./perm_values_AO.out',STATUS='UNKNOWN')
...
C***** Modification made by Andres Osorio: Sept. 2012 
C      Write to this file P(x|alfa) for a given permutation
C
            if (loop_index.gt.1) then 
              write(25,*)  loop_index, perm_pos, config_pos, counter,
     &          cross, sd
            endif
...
C*****Modification made by Andres Osorio: Sept. 2012 
      ...
      OPEN(UNIT=23,FILE='./details_AO.out',STATUS='UNKNOWN')
      
      do  perm_pos=1,num_per
        call give_permut(perm_id)
        do i=3,nexternal
          write(23,*) "next_perm > : association ", i, 
     &      inv_matching_type_part(2+ perm_id(matching_type_part(i)-2))
     &      , perm_id( matching_type_part(i)-2 ), perm_id(i-2)  
        enddo
      enddo
