...
void makePlots(const char *filename, const char * graph_title, const char * perfil ) {
  ...
  for(int k=0; k < ndataset; ++k) {
    ...
    if( k == 0 ) {
      ...
      ((TGraph*)allgraphs->At(k))->GetXaxis()->SetTitle("E( #nu ) [eV]");
  ...
  legend->AddEntry((TGraph*)allgraphs->At(0),"#nu_{e}","L");
  legend->AddEntry((TGraph*)allgraphs->At(1),"#nu_{#mu}","L");
  legend->AddEntry((TGraph*)allgraphs->At(2),"#nu_{#tau}","L");

  legend->Draw();

  /// All done!
  
  TString fullnameStr(filename);
  int pos0 = fullnameStr.First("/");
  int len0 = fullnameStr.Length();
  TString filenameStr( fullnameStr(pos0+1, len0) );
  int pos1 = filenameStr.First("_");
  TString subDir = filenameStr(0,pos1);
  int pos2 = filenameStr.First(".");
  TString fname  = filenameStr(0,pos2);

  TString mainDir("Graficas_Perfil_");
  TString perfilStr( perfil );

  TString outputPNG = mainDir + perfilStr + TString("/") + subDir
    + TString("/png") + TString("/") + fname + TString(".png");

  TString outputEPS = mainDir + perfilStr + TString("/") + subDir 
    + TString("/eps") + TString("/") + fname + TString(".eps");

  
  canvas->Print( outputPNG.Data() );
  canvas->Print( outputEPS.Data() );
