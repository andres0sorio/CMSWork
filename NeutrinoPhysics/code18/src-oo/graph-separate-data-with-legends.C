{
  ...
  //Add Legends:
  TLegend * legend = new TLegend(0.15,0.67,0.45,0.86,NULL,"brNDC");
  //Legend options
  ...
  legend->AddEntry(graphT1,"6 columns data","P");
  legend->AddEntry(graphT2,"4 columns data","P");
  legend->AddEntry(graphT3,"3 columns data","P");
  legend->AddEntry(graphT4,"2 columns data","P");
  ...
  for(int k=0; k < ndataset; ++k) {
    ...
    if( k == 0 ) {
      ...
      ///Axis titles
      // use # as in Latex \ for example \alpha \cdot == #alpha #cdot
      // use th ^ as in Latex for superscript and _ for subscript
      //
      ((TGraph*)allgraphs->At(k))->GetXaxis()->SetTitle("Log #nu [Hz]");
      ((TGraph*)allgraphs->At(k))->GetXaxis()->CenterTitle(true);
      ((TGraph*)allgraphs->At(k))->GetYaxis()->SetTitle("Log #nuF(#nu) [ erg cms^{-2} s^{-1} ]");
