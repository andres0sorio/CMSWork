...
//=============================================================================
// Clone method implementation: this is a formality
//=============================================================================
...
double m_Numu_integral_dxdy::DoEval(double y) const {
  ...
  double ff = 0.5 * Rmu ( x1 ) * dFnub( x2, par) * sigma_CC * dsigmady( x3 );

  sigma_CC_log10 = antinu_xsec_interp->evaluateCC( log10( m_x) );
  sigma_CC = pow( 10.0, sigma_CC_log10);
  
  double gg = 0.5 * Rmu ( x1 ) * dFnub( x2, par) * sigma_CC * dsigmady( x3 );
...
double m_Nutau_integral_dxdydz::DoEval(double z) const{
  ...
  double ff = 0.5 * Rmu ( xx ) * dFnub( x2, par) * fBLimOne( yy ) * sigma_CC * dsigmady( x3 );
  
  sigma_CC_log10 = antinu_xsec_interp->evaluateCC( log10( m_x ) );
  sigma_CC = pow( 10.0, sigma_CC_log10);
  
  double gg = 0.5 * Rmu ( xx ) * dFnub( x2, par) * fBLimOne( yy ) * sigma_CC * dsigmady( x3 );
