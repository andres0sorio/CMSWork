...
const Int_t kMaxparticles = 4000;
...
public :
  ...
  PythiaAnalysis(const char * fname, const char * opt, int type);
...
PythiaAnalysis::PythiaAnalysis(const char * fname, const char * opt, int type)
{
  ...
  //m_out = new std::ofstream("pythia8-tree-analysis.txt");
  
  m_root_out = new RootOutput( opt, type);
