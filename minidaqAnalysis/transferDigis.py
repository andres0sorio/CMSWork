...
prevstep =  type + '.step1.'+ options.runid
...
if mwgrid == 'minidaq':
	targetdir   = '/castor/cern.ch/user/c/ccmuon/RPC/minidaq/' + str(runid) + '/'
else:
	targetdir   = '/castor/cern.ch/user/c/ccmuon/RPC/GlobalRuns/' + mwgrid + '/' + str(runid) + '/'

outputdir   = '/castor/cern.ch/user/a/aosorio/gridfiles/ganga/'
outfilename = str(runid)+'/list_of_digis.txt'

excode = os.path.isdir(str(runid))
if excode != True:
	os.mkdir(str(runid))
...
for jb in jobs(jobid).subjobs:
	...
	if jbs == 'completed':
		...
		tmp2   = tmp1[-1]
		tmp3   = tmp2.split('.')
		pos    = len(tmp3[1]) - len(options.runid)
...
myjob.name = type + '.transfer.' + options.runid
