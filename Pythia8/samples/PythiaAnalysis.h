...
public :
  ...
  PythiaAnalysis(const char * fname, const char * outfname );
  ...
  virtual void     Loop( Long64_t ievent );
  virtual void     Loop( Long64_t firstevent, Long64_t lastevent );
  
  TList * v_FinalState;
  TList * v_Neutrinos;
  TList * v_Neutralinos;
  TList * v_InitialState;
  ...
  virtual void SetInitialState( int );
    
  int m_InitialStatePDG;
...
PythiaAnalysis::PythiaAnalysis(const char * fname, const char * outfname )
{
  ...
  //Aqui se crea el contenedor para almacenar las particulas de estado final
  v_FinalState = new TList(); 
  v_Neutrinos= new TList();
  v_Neutralinos = new TList();
  v_InitialState = new TList();
  
  histos = new Histograms( outfname );
...
PythiaAnalysis::~PythiaAnalysis()
{
  ...
  delete v_FinalState;
  delete v_Neutrinos;
  delete v_Neutralinos;
  delete v_InitialState;
...
void PythiaAnalysis::SetInitialState( int pdg)
{
  
  m_InitialStatePDG = pdg;
