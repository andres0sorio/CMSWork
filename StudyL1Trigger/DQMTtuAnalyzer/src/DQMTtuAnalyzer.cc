#include "StudyL1Trigger/DQMTtuAnalyzer/interface/DQMTtuAnalyzer.h"

//
DQMTtuAnalyzer::DQMTtuAnalyzer(const edm::ParameterSet& iConfig){
...
void
DQMTtuAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if ( !gtRecord.isValid() ) {
    edm::LogError("DQMTtuAnalyzer") << "can't find L1GlobalTriggerRecord with label: " 
                                    << m_gtReadoutLabel << '\n';
    return;
  }
  ...
  if ( !emuTTRecord.isValid() ) {
    edm::LogError("DQMTtuAnalyzer") << "can't find L1GtTechnicalTriggerRecord (emulator) with label: " 
                                    << m_rpcTechTrigEmu << '\n';
    return;
  }
...
int  DQMTtuAnalyzer::discriminateGMT( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
...
void DQMTtuAnalyzer::discriminateDecision( bool data, bool emu , int indx ) 
{
...
void 
DQMTtuAnalyzer::beginJob(){

  edm::Service<TFileService> fservice;

    
 m_ttBitsDecisionData = fservice->make<TH1D>("TechTrigger.Bits.Data",
                                             "Technical Trigger bits : Summary",
                                              10, 23, 33 );

 m_ttBitsDecisionEmulator = fservice->make<TH1D>("TechTrigger.Bits.Emulator",
                                                 "Technical Trigger bits : Summary",
                                                 10, 23, 33 );
 for( int k=0; k < m_maxttBits; ++k) {
   ...
   m_bxDistDiffPac[k] = fservice->make<TH1D>(hname.str().c_str(),
   ...
   m_bxDistDiffDt[k] = fservice->make<TH1D>(hname.str().c_str(),
   ...
   m_dataVsemulator[k] = fservice->make<TH1D>(hname.str().c_str(),
 ...
 m_combinedTTbits = fservice->make<TCanvas>("CombinedTTbits", "Analysis area", 406, 253, 698, 498 );
  
  
}

void 
DQMTtuAnalyzer::beginRun(const edm::EventSetup& iSetup){
  
}


void 
DQMTtuAnalyzer::endJob() {

  //... Draw Data/emulation comparison
  TStyle * dqmTtuStyle = new TStyle("dqmTtuStyle","A simplified style");
  dqmTtuStyle->SetOptStat(11);
  dqmTtuStyle->cd();
  
  setCanvasOptions( m_combinedTTbits );
  
  m_combinedTTbits->cd();
  m_ttBitsDecisionData->Draw();
  m_ttBitsDecisionEmulator->SetLineColor(2);
  m_ttBitsDecisionEmulator->Draw("same");
  m_combinedTTbits->Write();
  
}

void 
DQMTtuAnalyzer::setCanvasOptions( TCanvas * canvas) {
...
DEFINE_FWK_MODULE(DQMTtuAnalyzer);
