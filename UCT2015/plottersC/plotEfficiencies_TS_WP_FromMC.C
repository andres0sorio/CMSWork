void makePlots() {
  ...
  //Pt
  if( 1 ) {
    makePlots("recoPt", "recoPt > 0.0", "50,0,140", 0.0, 100);
  }
...
void makePlots( const char * varname, const char * varSel, const char * binning, float minPU, float maxPU ) {
  ...
  if ( 1 ) { 
    ...
    input_data = TString("../L1Tree_Htt_MC_RAWRECO_MC_VisTau.root");
    path << "./uct15-plots/efficiency/Taus/withPUC/isoStudies";
  ...
  L1UCTEfficiency * eg1   = new L1UCTEfficiency(input_data.Data(),"rlxVisTauSt1Efficiency/Ntuple");
  ...
  for( int k = 0; k < max; ++k ) {
    ...
    saveAs << path.str() << "/png/rlx_tau_eff_MC_trg20_" << varname  << "_" << minPU << "_" << maxPU << "_" << k << "_" << PUcut << ".png";
    eg1->ComparePlots(v_efficiencies,"EG Efficiency", saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path.str() << "/eps/rlx_tau_eff_MC_trg20_" << varname  << "_" << minPU << "_" << maxPU << "_" << k << "_" << PUcut << ".eps";
