void plotResolution( ) {
  ...
  plotResolution("Pt", 0,200); //0 - 200 Pt Range
  plotResolution("Eta",0,200);
  plotResolution("Phi",0,200);
...
void plotResolution( char* var, float xmin, float xmax ) {
  ...
  //TFile * f1 = new TFile("../L1Tree_TauX_Skim_2012C_Test.root" );

  TFile * f1 = new TFile("../L1Tree_MuTau_2012B_RAWRECO.root" );
  ...
  for( int k = 0 ; k < 1; ++k ) {
    ...
    sprintf(cuts, " l1gMatch > 0 && abs(l1gEta) < 1.4  && recoPt > %d && recoPt < %d", xMin, xMax);
    ...
    Stage1Taus->Draw( histoDraw + TString(st1HistoName) + TString("(50,-1.0 , 1.0)"), cuts, "");
    TH1F* h1 = (TH1F*)gDirectory->Get( st1HistoName )->Clone();

    std::cout << h1 << std::endl;
    Stage1Histos->Add( h1 );

    Stage2Taus->Draw( histoDraw + TString(st2HistoName) + TString("(50,-1.0 , 1.0)"), cuts, "");
    ...
    saveAs.str("");
    saveAs << path << "/eps/rlx_tau_reso_DiTau_" << var << "_" << xMin << "_" << xMax << "_dcm_" << k << "_" << ".eps";
    canvas->SaveAs( saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path << "/png/rlx_tau_reso_DiTau_" << var << "_" << xMin << "_" << xMax << "_dcm_" << k << "_" << ".png";
    canvas->SaveAs( saveAs.str().c_str() );
