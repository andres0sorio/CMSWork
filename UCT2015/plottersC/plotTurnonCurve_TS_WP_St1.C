...
void plotTurnonCurve(char* var, double min, double max, int NPVmin=0, int NPVmax=50) {
  ...
  TString path("./uct15-plots/turnon/Taus/withPUC/isoStudies");
  ...
  if( 1 ) {
    ...
    f1 =  TFile::Open("../L1Tree_DiTaus_Resolution_Stg12.root");

  } 

  TTree* tree1 = (TTree*) gDirectory->Get("rlxTauSt1Efficiency/Ntuple");
  ...
  for( int k = 0; k < max_sel; ++k ) {
    ...
    saveAs.str("");
    saveAs << path << "/eps/rlx_tau_eff_st1_trg20_WP_" << varName << "_" << NPVmin << "_" << NPVmax << "_" << k << "_" << PUcut << ".eps";
    canEffIso->SaveAs( saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path << "/png/rlx_tau_eff_st2_trg20_WP_" << varName << "_" << NPVmin << "_" << NPVmax << "_" << k << "_" << PUcut << ".png";
    canEffIso->SaveAs( saveAs.str().c_str() );
