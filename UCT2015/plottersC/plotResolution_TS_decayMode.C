...
void plotResolution( ) {
  ...
  if (region == 2 ) {
    ...
    plotResolution("Pt", 0, maxPt, "( abs(recoEta) < 1.4 )", "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region);
    plotResolution("Eta",0, maxPt, "( abs(recoEta) < 1.4 )", "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region);
    plotResolution("Phi",0, maxPt, "( abs(recoEta) < 1.4 )", "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region);
    
  } else {
    
    plotResolution("Pt", 0, maxPt, "( abs(recoEta) > 1.6 && abs(recoEta) < 2.4 )" , "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region);
    plotResolution("Eta",0, maxPt, "( abs(recoEta) > 1.6 && abs(recoEta) < 2.4 )" , "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region);
    plotResolution("Phi",0, maxPt, "( abs(recoEta) > 1.6 && abs(recoEta) < 2.4 )" , "(50,-2.0 , 2.0)", "MuTau_MTcut_EC", region);
...
void plotResolution( char* var, float xmin, float xmax, const char * region, const char *xbinning, const char * skim, int regionCode) {
  ...
  TFile * f1 = new TFile("../L1Tree_MuTau_MTCut_High.root" );
  ...
  for( int k = 0 ; k < 3; ++k ) {
    ...
    saveAs.str("");
    saveAs << path << "/eps/rlx_tau_reso_" << skim << "_" << var << "_" << xMin << "_" << xMax << "_dcm_" << k << "_" << regionCode << ".pdf";
    canvas->SaveAs( saveAs.str().c_str() );
    
    saveAs.str("");
    saveAs << path << "/png/rlx_tau_reso_" << skim << "_" << var << "_" << xMin << "_" << xMax << "_dcm_" << k << "_" << regionCode << ".png";
    canvas->SaveAs( saveAs.str().c_str() );
