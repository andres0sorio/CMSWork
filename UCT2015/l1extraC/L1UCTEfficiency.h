...
class L1UCTEfficiency : public Histograms {
 public :
  ...
  Float_t         dr03CombinedEt; // only for EG analysis
  Float_t         dr03EcalRecHitSumEt; // only for EG analysis
  Float_t         dr03HcalTowerSumEt; // only for EG analysis
  Float_t         dr03TkSumPt; // only for EG analysis
  Float_t         emFraction;
  Float_t         decayMode;
  Float_t         hcal;
  Float_t         evt;
  ...
  L1UCTEfficiency(const char *infile, const char *path, const char *type );
...
L1UCTEfficiency::L1UCTEfficiency(const char * infile, const char * path, const char *type) : Histograms()
{
  ...
  if ( m_type.compare("EG") == 0 )
    m_isEG = true;
  else if ( m_type.compare("Tau") == 0 )
    m_isTau = true;
  else
    std::cout << "L1UCTEfficiency > No EG/Tau object" << std::endl;
...
void L1UCTEfficiency::Init(TTree *tree)
{
  ...
  if ( m_isEG ) {
    ...
    fChain->SetBranchAddress("emFraction", &emFraction, &b_emFraction);
    fChain->SetBranchAddress("decayMode", &decayMode, &b_decayMode);
    fChain->SetBranchAddress("hcal", &hcal, &b_hcal);
  } else
    std::cout << "L1UCTEfficiency > No EG/Tau object - branches not initialized for those objects" << std::endl;
