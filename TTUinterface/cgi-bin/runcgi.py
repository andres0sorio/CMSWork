...
def main():
    ...
    address_opt = ("Set the adapter on which the server listens, by address. "
        "Defaults to 'localhost'. '0.0.0.0' means listen on all available adapters.")
    ...
    parser.add_option("-a", "--address", default='localhost', help=address_opt)
    parser.add_option("-p", "--port", type='int', default=5000, help=port_opt)
    ...
    cgi_dirs = ['/home/aosorio/Exploration/Python/CGI/cgi-bin']
    print cgi_dirs
    print cgi_target
                
    httpd = BaseHTTPServer.HTTPServer((options.address, options.port), handler)

    thread.start_new_thread(httpd.serve_forever, ())

    url = 'http://%s:%d/%s' % (options.address, httpd.server_port, cgi_target)
