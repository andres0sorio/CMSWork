...
# http://cern.ch/ganga
#
# $Id: ganga_setup.py,v 1.1 2007/11/16 18:05:27 amuraru Exp $
#
...
def usage(error=None):
	print >> sys.stderr, ''' synopsis: ganga_setup.py --version=VERSION --interactive --experiment=EXP --max-version=VER cmd
	...
	--max-version=VER specifies a maximum version to allow (also disables higher versions from being displayed interactively)
...
def main():
	...
	prefixStr = '/home/aosorio/hepsoft/ganga/install'
	versionStr = None
	interactive = False
	showAll	= False
	experiment = 'generic'
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "", ["version=","interactive","show-all","experiment=","max-version="])
	...
	# require version isn't > maxVersion is it was set
	if maxVersion and (versionStr != 'latest' and versionStr != 'last' \
			   and versionStr != 'default' \
			   and versionStr != 'quit'):
		if _relcmp(versionStr,maxVersion) > 0:
			print 'ERROR: Version requested ("%s") is greater '\
			      'than maximum allowed ("%s").' \
			      % (versionStr,maxVersion)
			sys.exit(2)

	defaultVersion,publicVersions, allVersions = get_versions(prefixStr,maxVersion)
	...
	if interactive:
		...
		while not selected:
			if experiment == 'lhcb':
				print >> sys.stderr, 'GangaEnv is depricated! You should be using "SetupProject Ganga".'
	...
	setupcmds = { 'lhcb' : { 'sh': """export PATH=%(PATH)s ; export GANGA_CONFIG_PATH=%(INSTALL_PREFIX)s/config/GangaLHCbRoot.ini:%(INSTALL_PREFIX)s/config/GangaLHCb%(RELEASE)s.ini:GangaLHCb/LHCb.ini""",
								'csh': """setenv PATH %(PATH)s ; setenv GANGA_CONFIG_PATH %(INSTALL_PREFIX)s/config/GangaLHCbRoot.ini:%(INSTALL_PREFIX)s/config/GangaLHCb%(RELEASE)s.ini:GangaLHCb/LHCb.ini"""
					},
					'atlas' : { 'sh': """alias ganga=%(BIN_DIR)s/ganga ; export GANGA_CONFIG_PATH=%(INSTALL_PREFIX)s/config/GangaAtlas%(RELEASE)s.ini:GangaAtlas/Atlas.ini ; export PANDA_URL=http://pandaserver.cern.ch:25080/server/panda ; export PANDA_URL_SSL=https://pandaserver.cern.ch:25443/server/panda """,
							'csh': """alias ganga %(BIN_DIR)s/ganga ; setenv GANGA_CONFIG_PATH %(INSTALL_PREFIX)s/config/GangaAtlas%(RELEASE)s.ini:GangaAtlas/Atlas.ini ; setenv PANDA_URL http://pandaserver.cern.ch:25080/server/panda ; setenv PANDA_URL_SSL https://pandaserver.cern.ch:25443/server/panda """ },
	...
	# Set a different site specific ini file depending on version

	if _relcmp(versionStr,'5.5.22-pre')>=0:
		vars['RELEASE']='-v5522'
	elif _relcmp(versionStr,'5.5.0-pre')>=0:
		vars['RELEASE']='-v550'
	elif _relcmp(versionStr,'5.3.1-pre')>=0:
		vars['RELEASE']='-v531'
	elif _relcmp(versionStr,'5.3.0-pre')>=0:
		vars['RELEASE']='-v53'
	elif _relcmp(versionStr,'5.2.3-pre')>=0:
		vars['RELEASE']='-v523'
	elif _relcmp(versionStr,'5.1.5-pre')>=0:		
		vars['RELEASE']='-v515'
	elif _relcmp(versionStr,'5.1.0-pre')>=0:		
		vars['RELEASE']='-v51'
	elif _relcmp(versionStr,'5.0.0-beta1')>=0:		
		vars['RELEASE']='-v5'
	elif _relcmp(versionStr,'4.4.0-beta1')>=0:
		vars['RELEASE']='-v44'
	else:		
		vars['RELEASE']=''
...
def get_versions(prefixStr,maxVersion):
	...
	if maxVersion:
		all = [a for a in all if _relcmp(a,maxVersion) <= 0]	
	all.sort(_relcmp)
	...
	if ENABLE_HOTFIX:
	  #take last hotfix as default version if exists
	  if public[-2].find('hotfix'):
		if public[-1].split('-')[0] == public[-2].split('-')[0]:
			defaultVer = public[-2]
	return defaultVer,public,all
